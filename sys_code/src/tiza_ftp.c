...
//	
//		
//	L218SendAtCmd(AT_CPIN_INDEX,NULL,0,(uint8 *)OK_ACK,2, 1);	//-检查SIM
//		
...
static uint8 FtpSAPBR(void)
{
	...
	L218SendAtCmd(AT_SAPBR_INDEX,(uint8*)FTP_TYPEOFINTERCONNECT,21,(uint8 *)OK_ACK,2, 1);
	//L218SendAtCmd(AT_SAPBR_INDEX, NULL, 0,(uint8 *)OK_ACK,2);
	...
	if(g_at_cmd_struct[AT_SAPBR_INDEX].exe_flag == EXE_OK)
		...
		return res;
	//-2.Set APN for FTP	
	L218SendAtCmd(AT_SAPBR_INDEX,(uint8*)FTP_SETAPN, 18, (uint8 *)OK_ACK, 2, 1);
	...
	if(g_at_cmd_struct[AT_SAPBR_INDEX].exe_flag == EXE_OK)
		...
		return res;	
	//-3.Open bearer	
	L218SendAtCmd(AT_SAPBR_INDEX,(uint8*)FTP_OPENBEARER,4,(uint8 *)OK_ACK, 2, 1);
	//-暂时不考虑应答?是否有应答待验证??
	if(g_at_cmd_struct[AT_SAPBR_INDEX].exe_flag == EXE_OK)
		res = RES_TRUE;
	else
...
static uint8 FtpFTPTYPE(void)
{
	...
	L218SendAtCmd(AT_FTPTYPE_INDEX,NULL,0,(uint8 *)OK_ACK,2, 1);
...
static uint8 FtpFTPSERV(void)
{
	...
	L218SendAtCmd(AT_FTPSERV_INDEX,NULL,0,(uint8 *)OK_ACK,2, 1);	
...
static uint8 FtpFTPPORT(void)
{
	...
	L218SendAtCmd(AT_FTPPORT_INDEX,NULL,0,(uint8 *)OK_ACK,2, 1);	
...
static uint8 FtpFTPUN(void)
{
	...
	L218SendAtCmd(AT_FTPUN_INDEX,NULL,0,(uint8 *)OK_ACK,2, 1);	
...
static uint8 FtpFTPPW(void)
{
	...
	L218SendAtCmd(AT_FTPPW_INDEX,NULL,0,(uint8 *)OK_ACK,2, 1);	
...
static uint8 FtpFTPSIZE(void)
{
	...
	L218SendAtCmd(AT_FTPSIZE_INDEX,NULL,0,(uint8 *)ack,10, 1);
...
//	uint8 TEMP_file_size[32] = "2,1024\x0d";	
  uint8  i = 0,cmp_data[20];// = {"+FTPGET:1,1"};
	...
	L218SendAtCmd(AT_FTPGETNAME_INDEX,NULL,0,(uint8 *)OK_ACK,2, 1);	
	...
	res=RES_FALSE;
	L218SendAtCmd(AT_FTPGETPATH_INDEX,NULL,0,(uint8 *)OK_ACK,2, 1);	
	...
	res = FtpFTPSIZE();
//	res=RES_FALSE;
//	L218SendAtCmd(AT_FTPSIZE_INDEX,NULL,0,(uint8 *)OK_ACK,2, 1);	
//	if(g_at_cmd_struct[AT_FTPSIZE_INDEX].exe_flag == EXE_OK)
//		res = RES_TRUE;
//	else
//		return res;
	...
	res=RES_FALSE;
	L218SendAtCmd(AT_FTPGET1_INDEX,NULL,0, cmp_data, 11, 1);	
...
static uint8 FtpDLAFile(void)
{
  ...
  uint8  i = 0,TEMP_OK_ACK[20];// = {"+FTPGET:1,1"};
	...
	L218SendAtCmd(AT_FTPGET2_INDEX,TEMP_file_size,7,(uint8 *)TEMP_OK_ACK,11, 1);
...
static uint8 FtpFTPQUIT(void)
{
	...
	L218SendAtCmd(AT_FTPQUIT_INDEX,NULL,0,(uint8 *)OK_ACK,2, 1);
...
static void FtpCBearer(void)
{
	...
	//-1.Close bearer
	L218SendAtCmd(AT_SAPBR_INDEX,temp_data,StrLen(temp_data,0),(uint8 *)OK_ACK,2, 1);	
}
...
RETURN_LAB:	
	if(res == RES_FALSE) 
	{
		...
		#ifdef FTP_DEBUG
			LocalUartFixedLenSend((uint8*)"!!!warn ftp_upgrade_fail\r\n", 26);
		#endif
	}
