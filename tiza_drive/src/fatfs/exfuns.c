...
u8 exfuns_init(void)
{
	...
	for(j=0;j<FIL_NUM;j++)
	{
		g_file_struct.count[j] = 0; 
		g_file_struct.file[j]=(FIL*)mymalloc(SRAMIN,sizeof(FIL));	//为file申请内存
		if(!g_file_struct.file[j])break;
	}
	g_file_struct.file[FIL_NUM]=(FIL*)mymalloc(SRAMIN,sizeof(FIL));	//为file[5]申请内存
	fatbuf=(u8*)mymalloc(SRAMIN,512);						//为fatbuf申请内存
	if(i==_VOLUMES && j==FIL_NUM && g_file_struct.file[FIL_NUM] && fatbuf)return 0;  //申请有一个失败,即失败.
	else return 1;	
}
